...
3. 如果没有 `mx-skills`，再使用本 skill 内的 `scripts/stock_cli.py`、AkShare 接口或 BaoShare 作为回退。
4. 如果缺少 `akshare` / `baostock` 等本地依赖，再按需安装。
5. 只拉取回答当前问题所需的数据。
6. 按问题选择分析视角：
   - 筹码分布 / 成本结构
   - 日线、周线、月线三周期共振
...
- 回答里不必铺陈数据源切换过程，除非切换影响了数据口径、时效性或结论置信度。
...
如果某个数据源不稳定，按 `mx-skills -> AkShare -> BaoShare` 的顺序切换，不要直接中断。优先给出结构级判断，而不是依赖单一脆弱接口追求伪精确。
...
如果用户尚未配置 `mx-skills`，建议其前往 [https://ai.eastmoney.com/mxClaw](https://ai.eastmoney.com/mxClaw) 下载并完成配置，以便优先获得更完整的个股数据。
